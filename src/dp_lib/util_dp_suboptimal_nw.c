...
int find_index(int value, Constraint_list *CL)
{
   int i;
   for (i=0; i<CL->n_medoids; i++)
   {
	 if (CL->medoids_set[i] == value)
	 {
...
Constraint_list *ProbaMatrix2CLMAX (Alignment *A, int *ns, int **ls, int NumMatrixTypes, int NumInsertStates, float *forward, float *backward, float thr, Constraint_list *CL) {
  float totalProb;
  ...
  //fprintf(stdout, "s1 cluster \"%d\"\n",CL->clusters_map[s1]);
  //fprintf(stdout, "s2 cluster \"%d\"\n",CL->clusters_map[s2]);
  //fprintf(stdout, "s1:%d medoid \"%d\"\n",s1, find_index(s1, CL));
  //fprintf(stdout, "s2:%d medoid \"%d\"\n",s2, find_index(s2, CL));

	if(CL->clustering) {
		if((CL->clusters_map[s1] != CL->clusters_map[s2]) && (find_index(s1, CL) != 1 || find_index(s2, CL) != 1)) {
			//fprintf(stdout, "Skipping %d-%d pairwise | cluster(%d-%d) | medoid(%d-%d)\n",s1, s2, CL->clusters_map[s1], CL->clusters_map[s2], find_index(s1, CL), find_index(s2, CL));
			return A->CL;
		}
	}

  //fprintf(stdout, "Generating %d-%d pairwise | cluster(%d-%d) | medoid(%d-%d)\n",s1, s2, CL->clusters_map[s1], CL->clusters_map[s2], find_index(s1, CL), find_index(s2, CL));
...
Constraint_list *ProbaMatrix2CL (Alignment *A, int *ns, int **ls, int NumMatrixTypes, int NumInsertStates, float *forward, float *backward, float thr, Constraint_list *CL)
{
  ...
  if(CL->clustering) {
  	return ProbaMatrix2CLMAX (A, ns, ls, NumMatrixTypes, NumInsertStates, forward, backward, thr, CL);
  }
