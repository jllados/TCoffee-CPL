		...
		overaln_P3=0;
	       ...
	       if (!strm (dump, "no"))
		 {
	       ...
	       get_cl_param(\
			    ...
			    /*OPTIONAL?*/ OPTIONAL       ,\
			    /*MAX Nval*/  2             ,\
			    /*DOC*/       "-clustering must be a float between [0,1] 0.75-(accurate) 0.5-(balanced) 0.25-(fastest)"           ,\
...
/*******************************************************************************************************/
		...
		* Note that when running master, the library must be relaxed in order to compensate for missing pairs
		...
		if(clustering>1.0) {clustering=1;}
		if(clustering<0.0) {clustering=0;}
		...
		if(CL->clustering) {
	       ...
	       if ( !do_convert && !strm (weight, "no")) {
			...
			int i, j, ivar;
			char pid_dist[128], pid_clusters[128], pid_medoid[128], cmmd[80];
			ofstream myfile;
			string svar;
			
			snprintf(pid_dist, sizeof(pid_dist), "%d.dist", getpid());
			//fprintf(stdout,"|- Saving distance matrix %s\n",pid_dist);

			//OUTPUT DIST MATRIX ***
			myfile.open(pid_dist);
			...
			for(i=0;i<S->nseq;i++) {
				...
				for(j=0;j<S->nseq;j++) {
					if(j==S->nseq-1)
						myfile << (float)(CL->DM)->distance_matrix[i][j];
					else
						myfile << (float)(CL->DM)->distance_matrix[i][j] << "\t";
				}
				myfile << "\n";
			...
			//GENERATE FILES ***
			fprintf(stdout, "Number of Clusters: %d\n", int(round(S->nseq*CL->clustering)));
			
			sprintf(cmmd, "Rscript findclusters.R %d.dist %d %d", getpid(), int(round(S->nseq*CL->clustering)), getpid()); 
			system(cmmd); //generate pid.{newick, clusters, medoid} 
		
			//LOAD HCUST TREE ***
			sprintf(use_tree, "%d.newick", getpid());
			//fprintf(stdout,"|- Loading tree: %s\n", use_tree);
			if (   !use_tree && check_file_exists (tree_file))vremove (tree_file);
			else if ( !use_tree || (use_tree && strm (use_tree, "default")));
			else sprintf ( tree_file, "%s", use_tree);

			//LOAD CLUSTERS ***
			snprintf(pid_clusters, sizeof(pid_clusters), "%d.clusters", getpid());
			//fprintf(stdout,"|- Loading clusters: %s\n",pid_clusters);
		
			ifstream in_clusters; 
			in_clusters.open(pid_clusters); 

			in_clusters >> ivar;
			CL->clusters_map=(int*)malloc(ivar*sizeof(int));
			while (in_clusters >> svar >> ivar) {
				svar.erase(0, 1);
				svar.erase(svar.size() - 1);
				CL->clusters_map[name_is_in_list (svar.c_str(), (CL->S)->name, (CL->S)->nseq, 100)]=ivar;
			}

			in_clusters.close();

			//LOAD MEDOIDS ***
			snprintf(pid_medoid, sizeof(pid_medoid), "%d.medoid", getpid());
			//fprintf(stdout,"|- Loading medoids: %s\n",pid_medoid);

			ifstream in_medoids;
			in_medoids.open(pid_medoid);

			in_medoids >> ivar;
			CL->medoids_set=(int*)malloc(ivar*sizeof(int));
			CL->n_medoids=ivar;
			int med_counter=0;
			while (in_medoids >> svar) {
				svar.erase(0, 1);
				svar.erase(svar.size() - 1);
				CL->medoids_set[med_counter]=name_is_in_list (svar.c_str(), (CL->S)->name, (CL->S)->nseq, 100);
				med_counter++;
			}
			...
			/*int n_pw = 0, partial_pw;
			for(med_counter=0; med_counter<CL->n_medoids;med_counter++) {
				partial_pw = 0;
				for (i=0; i<S->nseq; i++) {
					if (CL->clusters_map[i] == med_counter+1) {
						partial_pw++;
					}
				}
				n_pw += (partial_pw*(partial_pw-1))/2;
			}
			n_pw += (CL->n_medoids*(CL->n_medoids-1))/2;
			fprintf(stdout, "total_pw: %d\n", n_pw);*/
		}
	       ...
	       else if ( do_convert && out_lib[0])
		 {
	        ...
	        */
	        ...
	        //fprintf(stderr, "(CL->S)->nseq: %d | CL->ne: %d | CL->M: %d | do_convert: %d | n_list: %d\n", (CL->S)->nseq, CL->ne, CL->M, do_convert, n_list);
...
/*******************************************************************************************************/
	      ...
	      else
		{
	      ...
	      if(CL->clustering) {remove(use_tree);}
